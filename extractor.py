...
HOW IT WORKS:
-------------
Python code is just text, but Python's `ast` module can parse it
into a tree structure (Abstract Syntax Tree). Each node in the tree
represents a piece of code:

    def greet(name):        <-- FunctionDef node
        return f"Hi {name}" <-- Return node with FormattedValue
...
-------------------------
Regex can find "def foo(" but can't understand:
- Is this inside a class? (method vs function)
- What are the decorators?
- What does it return?
- Is it nested inside another function?

AST gives us all of this context.
"""
